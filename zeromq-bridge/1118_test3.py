...
def connect_mqtt():
    ...
    client.connect(mqtt, int(mqtt_port))  # Ensure port is integer
    return client

def publish(topic, client, message):
    result = client.publish(topic, message)
    if result.rc == mqtt_client.MQTT_ERR_SUCCESS:  # Updated for paho-mqtt 2.x
...
def json_msg(name, time, result):
    ...
    for i in range(len(result_list)):
        ...
        data["CCTVDetectionReport"] = {
            "CameraNo": str("CCTV_") + str(name),
            "ClassType": result_list[i]["classType"],
            "Box": result_list[i]["box"],
        ...
        client = connect_mqtt()
        client.loop_start()
        publish(topic, client, data_json)  # Ensure `topic` is passed
        client.loop_stop()
...
if __name__ == "__main__":
    ...
    topic = "DataTopic"  # Ensure a valid topic is set
